...
const fetchRequests = async (status = null) => {
  ...
  try {
    const response = await fetch(`${apiBaseUrl}/getMyBookListings?id=${authState.user.id}&status=${status || route.query.status || 'all'}`, {
      credentials: 'include'
    })
    if (!response.ok) {
    ...
    console.log('Fetched listings:', requests.value)
  } catch (error) {
    console.log('Error fetching book listings:', error.message || error);
  }
}
